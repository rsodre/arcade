...
export function subscribeToTokenUpdatesClause(collectionAddress: string, tokenId: BigNumberish | undefined) {
  return KeysClause(
    [ModelsMapping.Order],
    [undefined, addAddressPadding(collectionAddress), tokenId?.toString(), undefined],
  ).build();
}
function getOrderBaseClause(collectionAddress: string, tokenId: BigNumberish | undefined, category: OrderCategory) {
  return AndComposeClause([
    ...
    MemberClause(ModelsMapping.Order, "status", "Eq", OrderStatus.Placed.toString()),
  ]);
}
...
export function getTokenQuery(collectionAddress: string, tokenId: string) {
  return new ToriiQueryBuilder()
    .withClause(
      KeysClause([ModelsMapping.Order], [undefined, addAddressPadding(collectionAddress), tokenId, undefined]).build(),
    )
    .includeHashedKeys()
...
export function getTokenOrders(collectionAddress: string, tokenId: string) {
  return new ToriiQueryBuilder()
    .withClause(getOrderBaseClause(collectionAddress, tokenId, OrderCategory.Buy).build())
...
export function getListedTokensForCollection(collectionAddress: string) {
  return new ToriiQueryBuilder()
    .withClause(getOrderBaseClause(collectionAddress, undefined, OrderCategory.Sell).build())
    .addOrderBy("expiration", "Asc")
    .withEntityModels([ModelsMapping.Order])
    .includeHashedKeys();
}
...
export function isTokenListed(collectionAddress: string, tokenId: string) {
  return new ToriiQueryBuilder()
    .withClause(getOrderBaseClause(collectionAddress, tokenId, OrderCategory.Sell).build())
    .withEntityModels([ModelsMapping.Order])
    .includeHashedKeys();
}
