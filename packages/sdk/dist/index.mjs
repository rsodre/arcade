...
var DojoEmitterProvider = ApplyEventEmitter(DojoProvider);
...
var AchievementModel = class _AchievementModel {
  ...
  type = MODEL_NAME2;
  static from(model) {
    const worldAddress = addAddressPadding2(model.world_address);
    const namespace = shortString2.decodeShortString(`0x${BigInt(model.namespace).toString(16)}`);
    const id = shortString2.decodeShortString(`0x${BigInt(model.id).toString(16)}`);
    const published = !!model.published;
    const whitelisted = !!model.whitelisted;
    const karma = Number(model.karma);
...
var PinEvent = class _PinEvent {
  ...
  type = MODEL_NAME3;
  static from(model) {
    const playerId = addAddressPadding3(model.player_id);
...
var FollowEvent = class _FollowEvent {
  ...
  type = MODEL_NAME4;
  static from(model) {
    const follower = addAddressPadding4(model.follower);
    const followed = addAddressPadding4(model.followed);
    const time = Number(model.time);
...
var MemberModel = class _MemberModel {
  ...
  type = MODEL_NAME7;
  static from(model) {
    const id = addAddressPadding5(model.id);
...
import { shortString as shortString3 } from "starknet";
...
export {
  ...
  manifests,
  schema,
  manifest_sepolia_default as sepolia,
  setupWorld,
  manifest_slot_default as slot
};
