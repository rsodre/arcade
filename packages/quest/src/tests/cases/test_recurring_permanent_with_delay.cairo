//! Test case 6: Recurring Permanent with Delay quest
//!
//! Settings:
//! - Start > 0
//! - End = 0
...
//!
//! Expected behavior:
//! - Quest starts recurring at start timestamp
//! - Each cycle lasts duration seconds and repeats every interval seconds
//! - First cycle begins at start, subsequent cycles at start + n * interval
...
const QUEST_ID: felt252 = 'RECURRING-PERMANENT-DELAY';
...
fn NAME() -> ByteArray {
    "Recurring Permanent with Delay Quest"
}

fn DESCRIPTION() -> ByteArray {
    "A quest that starts later and repeats indefinitely"
}
...
fn test_recurring_permanent_with_delay_quest_creation() {
    ...
    // [Create] Quest with recurring permanent with delay settings
...
#[test]
fn test_recurring_permanent_with_delay_quest_multiple_completions() {
    let (world, systems, context) = spawn_game();
    let store = StoreTrait::new(world);
...
#[test]
fn test_recurring_permanent_with_delay_quest_claim_multiple_intervals() {
    let (world, systems, context) = spawn_game();
    let store = StoreTrait::new(world);
