//! Test case 8: Recurring Time-Limited with Delay quest
//!
//! Settings:
//! - Start > 0
//! - End > 0
...
//!
//! Expected behavior:
//! - Quest repeats within a specific time window
//! - Starts recurring at start timestamp and continues until end timestamp
//! - Each cycle lasts duration seconds and repeats every interval seconds
//! - Quest is only active during cycles that fall within [start, end) time window
...
const QUEST_ID: felt252 = 'RECURRING-TIME-LIMITED-DELAY';
...
#[test]
fn test_recurring_time_limited_with_delay_quest_creation() {
    let (world, systems, context) = spawn_game();
    let store = StoreTrait::new(world);

    // [Create] Quest with recurring time-limited with delay settings
...
#[test]
fn test_recurring_time_limited_with_delay_quest_multiple_completions() {
    let (world, systems, context) = spawn_game();
    let store = StoreTrait::new(world);
    ...
    let interval_id_0 = definition.compute_interval_id(START + 1000);
    ...
    let interval_id_1 = definition.compute_interval_id(START + INTERVAL + 1000);
...
#[test]
fn test_recurring_time_limited_with_delay_quest_claim_multiple_intervals() {
    let (world, systems, context) = spawn_game();
    let store = StoreTrait::new(world);
