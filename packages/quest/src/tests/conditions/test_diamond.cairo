//! Condition pattern 4: Diamond Pattern
//!
//! Pattern:
//!     A
//!    / \
...
//! - When B is completed, D's lock_count decrements to 1 (still locked)
//! - When C is completed, D's lock_count decrements to 0 (unlocked)
...
fn test_diamond_pattern_creation() {
    ...
    // [Create] Quest D with conditions B and C
    ...
    assert_eq!(definition_d.conditions.len(), 2, "Quest D should have 2 conditions");
}

#[test]
fn test_diamond_pattern_unlock_sequence() {
    let (world, systems, context) = spawn_game();
    let store = StoreTrait::new(world);
    ...
    // [Assert] B is completed, D is still locked (needs C)
    let definition_b = store.get_definition(QUEST_B);
    let completion_b = store.get_completion_or_new(context.player_id, @definition_b, 1000);
    let definition_d = store.get_definition(QUEST_D);
    let completion_d = store.get_completion_or_new(context.player_id, @definition_d, 1000);
    ...
    // [Assert] C is completed, D is now unlocked
    let definition_c = store.get_definition(QUEST_C);
    let completion_c = store.get_completion_or_new(context.player_id, @definition_c, 1000);
    let definition_d = store.get_definition(QUEST_D);
    let completion_d = store.get_completion_or_new(context.player_id, @definition_d, 1000);
