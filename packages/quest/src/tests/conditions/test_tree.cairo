//! Condition pattern 6: Tree with Multiple Levels
//!
//! Pattern:
//!     A
//!    / \
...
//! - Quests D and E start locked (lock_count = 1)
//! - When A is completed, B and C are unlocked
//! - When B is completed, D and E are unlocked
//! - Players can follow different paths
...
fn test_tree_pattern_creation() {
    ...
    // [Create] Quest D with condition B
    ...
    // [Create] Quest E with condition B
    let tasks_e = array![TaskTrait::new(TASK_E, TOTAL, "Task E Description")].span();
    systems
        .quester
        .create(
            id: QUEST_E,
    ...
    assert_eq!(definition_e.conditions.len(), 1, "Quest E should have 1 condition");
}

#[test]
fn test_tree_pattern_unlock_sequence() {
    let (world, systems, context) = spawn_game();
    let store = StoreTrait::new(world);
    ...
    let definition_e = store.get_definition(QUEST_E);
    let completion_e = store.get_completion_or_new(context.player_id, @definition_e, 1000);
    ...
    assert_eq!(completion_e.is_unlocked(), false, "Quest E should be locked");
    ...
    // [Assert] A is completed, B and C are unlocked
    let definition_a = store.get_definition(QUEST_A);
    let completion_a = store.get_completion_or_new(context.player_id, @definition_a, 1000);
    let definition_b = store.get_definition(QUEST_B);
    let completion_b = store.get_completion_or_new(context.player_id, @definition_b, 1000);
    ...
    assert_eq!(completion_e.is_unlocked(), false, "Quest E should still be locked");
    ...
    // [Assert] B is completed, D and E are unlocked
    let definition_b = store.get_definition(QUEST_B);
    let completion_b = store.get_completion_or_new(context.player_id, @definition_b, 1000);
    let definition_d = store.get_definition(QUEST_D);
    let completion_d = store.get_completion_or_new(context.player_id, @definition_d, 1000);
    ...
    assert_eq!(completion_e.is_unlocked(), true, "Quest E should be unlocked");

    // [Progress] Can now progress on D and E
    systems
        .quester
        .progress(player_id: context.player_id, task_id: TASK_D, count: COUNT, to_store: true);
    systems
        .quester
        .progress(player_id: context.player_id, task_id: TASK_E, count: COUNT, to_store: true);

    // [Assert] D and E have progress
    let advancement_d = store.get_advancement(context.player_id, QUEST_D, TASK_D, interval_id);
    let advancement_e = store.get_advancement(context.player_id, QUEST_E, TASK_E, interval_id);
    assert_eq!(advancement_d.count, COUNT, "Quest D should have progress");
    assert_eq!(advancement_e.count, COUNT, "Quest E should have progress");
}
