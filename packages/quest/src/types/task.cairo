...
mod tests {
    ...
    fn test_task_creation_new() {
        let quest = TaskTrait::new(ID, TOTAL, "DESCRIPTION");
        assert(quest.id == ID, 'Invalid ID');
        assert(quest.total == TOTAL, 'Invalid total');
        assert(quest.description == "DESCRIPTION", 'Invalid description');
    }
