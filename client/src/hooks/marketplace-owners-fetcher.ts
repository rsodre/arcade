...
export function useMarketOwnersFetcher({ project, address }: MarketOwnersFetcherInput) {
  const {
    status,
    isLoading,
    isError,
  ...
  const [ownersLoadingState, setOwnersLoadingState] = useState<{ [key: string]: OwnersLoadingState }>({});
  ...
  const updateLoadingState = useCallback((project: string, address: string, state: Partial<OwnersLoadingState>) => {
  ...
  const batchProcessOwners = useCallback(async (data: TokenBalance[]) => {
    const processed: { [address: string]: { [ownerAddress: string]: { balance: number; token_ids: string[]; username?: string } } } = { [address]: {} };
  ...
  const addOwners = useCallback((project: string, owners: { [address: string]: { [ownerAddress: string]: { balance: number; token_ids: string[]; username?: string } } }) => {
    useMarketplaceTokensStore.setState((state) => {
      ...
      for (const [collectionAddress, collectionOwners] of Object.entries(owners)) {
  ...
  const fetchOwnersImpl = useCallback(async (strategy: FetchStrategy = FetchStrategy.INITIAL, attemptNumber: number = 0) => {
    ...
    if (strategy === FetchStrategy.INITIAL && loadingState) {
      if (loadingState.isComplete && (now - loadingState.lastFetchTime) < CACHE_DURATION) {
  ...
  }, [project, address, addOwners, batchProcessOwners, startLoading, setSuccess, setLoadingProgress, createController, isMounted, getLoadingState, updateLoadingState]);

  const fetchOwners = useCallback(async (strategy: FetchStrategy = FetchStrategy.INITIAL) => {
    try {
      await withRetry(
        async (attemptNumber) => {
          if (attemptNumber > 0) {
  ...
  }, [fetchOwnersImpl, setRetryCount, setErrorMessage, setError, isMounted, updateLoadingState, project, address]);
