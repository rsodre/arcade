...
export interface MetadataFilterStore {
  ...
  toggleFilter: (collectionAddress: string, trait: string, value: string) => void;
  removeFilter: (collectionAddress: string, trait: string, value?: string) => void;
  clearFilters: (collectionAddress: string) => void;
  updateAvailableFilters: (collectionAddress: string, filters: AvailableFilters) => void;

  // Selectors
  getCollectionState: (collectionAddress: string) => CollectionFilterState | undefined;
...
export type UpdateMetadataIndex = (existingIndex: MetadataIndex, newTokens: Token[]) => MetadataIndex;
export type ExtractTokenAttributes = (token: Token) => TokenAttribute[];
export type CalculateFilterCounts = (metadataIndex: MetadataIndex, tokenIds?: string[]) => AvailableFilters;
export type ApplyFilters = (metadataIndex: MetadataIndex, activeFilters: ActiveFilters) => string[];
