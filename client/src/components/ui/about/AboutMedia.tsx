...
import { YoutubeEmbedIcon } from "./YoutubeIcon";

interface AboutMediaProps {
  items: string[];
}

export function AboutMedia({ items }: AboutMediaProps) {
  ...
  return (
    <div className="relative flex flex-col gap-2">
      ...
      <Carousel
        opts={{ dragFree: false, align: "start", loop: true }}
        setApi={setApi}
      >
...
const Image = ({ image, shift }: { image: string; shift: boolean }) => (
  ...
  >
    <img className="w-full h-full object-cover" src={image} alt="Media" />
  </div>
);
